...
{
    // Retrieve inputted key from the form
    $inputted_key = $_POST['key'];

    // Prepare SQL query to check if the key exists in the database
    $sql = "SELECT * FROM `admin` WHERE SecretKey = ?";
    ...
    $stmt->bind_param("s", $inputted_key);
    $stmt->execute();
...
?>
  ...
  <button class="btn1 btn-primary">Admin</button>
</div>
           ...
           <div class="row">
              ...
              <div class="col-12">
                <input type="submit" value="Admin Login" class="btn btn-primary" name="admin_login">
              </div>
